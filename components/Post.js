...
const Post = ({ post, modalPost }) => {
  ...
  const truncate = (string, length) =>
    string?.length > length
      ? string.substr(0, length - 1) + '...see more'
      : string;
  ...
  return (
    <div
      ...
      } space-y-2 py-2.5 border bprder-gray-300 dark:border-none`}
    >
      <div className="flex items-cneter px-2.5 cursor-pointer">
        <Avatar
          src={post.userImg}
          alt={post.username}
      ...
      {post.input && (
        <div className="px-2.5 break-all bd:break-normal">
          {modalPost || showInput ? (
